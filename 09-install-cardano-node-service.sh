...
WRAPPER="${SCRIPT_DIR}/cardano-node-start.sh"
mkdir -p "${HOME}/.config/systemd/user"

# Generate wrapper script (ExecStart must be a single executable, no shell line continuations)
cat > "${WRAPPER}" <<EOF
#!/usr/bin/env bash
# Source nix so it is available in the systemd environment
...
exec nix develop github:input-output-hk/ouroboros-leios#dev-testnet --command \\
  cardano-node run \\
